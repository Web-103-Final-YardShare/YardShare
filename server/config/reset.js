...
const createTables = async () => {
  try {
    ...
    // Listing photos table (replaces photo_urls array)
    await pool.query(`
...
const seedTestData = async () => {
  try {
    ...
    if (userResult.rows.length === 0) {
      ...
      `, [userId, 'Downsizing and decluttering! Check out my sales.', '407-555-1234', 'phone'])
    }
    ...
    const listing1 = await pool.query(`
      ...
      VALUES ($1, $2, $3, $4, CURRENT_DATE, '08:00', '13:00', $5, $6, $7, $8, true)
      RETURNING id
    `, [
      userId,
      categoryMap['Other'],
    ...
    await pool.query(`
      ...
      VALUES ($1, $2, true, 0)
    `, [listingId1, 'https://placehold.co/800x600?text=Yard+Sale+1'])

    const listing2 = await pool.query(`
      INSERT INTO listings (seller_id, category_id, title, description, sale_date, start_time, end_time, pickup_notes, location, latitude, longitude, is_active)
      VALUES ($1, $2, $3, $4, CURRENT_DATE + 1, '09:00', '15:00', $5, $6, $7, $8, true)
      RETURNING id
    `, [
      userId,
      categoryMap['Other'],
    ...
    await pool.query(`
      INSERT INTO listing_photos (listing_id, url, is_primary, position)
      VALUES ($1, $2, true, 0)
    `, [listingId2, 'https://placehold.co/800x600?text=Yard+Sale+2'])
    ...
    for (let i = 0; i < listing1Items.length; i++) {
      ...
      `, [listingId1, categoryMap[item.category], item.title, item.description, item.price, item.condition, item.image_url, i])
    }
    ...
    for (let i = 0; i < listing2Items.length; i++) {
      ...
      await pool.query(`
        INSERT INTO items (listing_id, category_id, title, description, price, condition, image_url, display_order)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      `, [listingId2, categoryMap[item.category], item.title, item.description, item.price, item.condition, item.image_url, i])
    }

    // Create an extra test user to simulate other users
