...
if (process.env.NODE_ENV !== 'production') {
  ...
  router.post('/dev/login/:username', async (req, res) => {
    try {
  ...
  router.post('/dev/password-login', async (req, res) => {
    try {
      ...
      if (!allowed.has(username)) {
        return res.status(403).json({ error: 'Dev password login allowed only for bryan or testuser' })
      }

      const key = username.toUpperCase()
      const expected = process.env[key] || process.env[`${key}_PASSWORD`]
      if (!expected) {
        return res.status(500).json({ error: `Missing env var ${key} or ${key}_PASSWORD for dev login` })
      }
